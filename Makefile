APP := shibboleth-sp
INSTALL_DIR := /opt/$(APP)
DEPLOY_DIR := $(INSTALL_DIR)/service
...
.PHONY: init
init: ## 配備先ディレクトリ群を作成する。
	sudo mkdir -p $(INSTALL_DIR) && sudo chown ec2-user:ec2-user $(INSTALL_DIR) && \
	mkdir -p $(DEPLOY_DIR) $(LOG_BASE_DIR)/shibboleth $(LOG_BASE_DIR)/httpd $(METADATA_DIR) $(CERTS_DIR)
...
.PHONY: gen_sp_cert
gen_sp_cert: ## SP証明書と秘密鍵を生成する（10年）。
	cd $(SOURCE_DIR) && ./scripts/gen-sp-cert.sh
...
erase: ## 当該機能で利用するDockerイメージを削除する。
	(cd $(DEPLOY_DIR) && docker compose down --rmi all || exit 0)
	docker image prune -f
